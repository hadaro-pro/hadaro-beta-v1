...
import {
  ...
  allLendingNftsByColAddressQuery,
  singleLendingNftByColAddressQuery,
} from "../../../../utils/queries";
import {
  parsePaymentToken,
  parseStandards,
...
export default async function handler(req, res) {
  try{
  const { collectionAddress, lenderAddress } = req.query;
  ...
  if (!compare)
    ...
    if(!lenderAddress) {
      const query2 = allLendingNftsByColAddressQuery(collectionAddress);
