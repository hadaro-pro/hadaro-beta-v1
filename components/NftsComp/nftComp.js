...
const NftDisplayComp = ({
  ...
  const performPush = (item) => {
    const ifCollectionExists = verifiedCollectionsArr.filter(
      (i) => i.collectionAddr === item.token_address
    );
  ...
  return (
    <>
      {nfts?.length === 0 ? (
        <div className={styles.noNFTPart}>
          <h1>
            No NFTs found on this network, try switching to another network😓...{" "}
          </h1>
        </div>
      ) : (
        <>
              ...
              <div key={index} className={styles.banner}>
                <div key={index} className={styles.nftcard}>
                  ...
                  <div className={styles.lowerPart}>
                    ...
                    <span>{el.name}</span>
                    <span>{/* {console.log(el.image)} */}</span>
                  </div>
                </div>
                <button
                  onClick={() => {
